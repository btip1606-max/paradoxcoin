...
The paradoxcoin Core daemon can be configured to act as a trusted "border
router", implementing the paradoxcoin wire protocol and relay, making
...
The ZeroMQ feature in paradoxcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building paradoxcoind:
...
For instance:
    ...
    $ paradoxcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/paradoxcoind.tx.raw \
...
These options can also be provided in paradoxcoin.conf.
...
From the perspective of paradoxcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into paradoxcoind directly. Furthermore, no information is
...
using. paradoxcoind appends an up-counting sequence number to each
