...
const char * const BITCOIN_CONF_FILENAME = "paradoxcoin.conf";
...
#else
    const char* pszModule = "paradoxcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "paradoxcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/paradoxcoin";
#else
    // Unix-like
    return pathRet / ".paradoxcoin";
#endif
#endif
}
